...
import { FaClock, FaFileInvoice, FaPlay, FaEye, FaRedo, FaFilter, FaDownload, FaCheck, FaTimes } from 'react-icons/fa'
...
const AdminFelPendingPage: React.FC = () => {
  ...
  const getStatusBadge = (status: FelInvoiceStatus) => {
    const statusConfig = {
      ...
      certified: { variant: 'default' as const, label: 'Certificada', icon: FaCheck },
      cancelled: { variant: 'destructive' as const, label: 'Anulada', icon: FaTimes },
  ...
  const processPendingInvoice = async (invoiceId: string) => {
    try {
      setProcessingInvoice(invoiceId)

      // Aquí iría la lógica para procesar la factura pendiente
      // Por ejemplo, generar XML, enviar a certificador, etc.
      await loadPendingInvoices()
    } catch (err) {
      console.error('Error procesando factura pendiente:', err)
      setError('Error al procesar la factura pendiente')
    } finally {
  ...
  const handleExport = async (format: 'csv' | 'excel') => {
    try {
      ...
      a.href = url
      a.download = `facturas-pendientes-fel-${new Date().toISOString().split('T')[0]}.${format}`
      document.body.appendChild(a)
      a.click()
      window.URL.revokeObjectURL(url)
      document.body.removeChild(a)
    } catch (err) {
      console.error('Error exportando facturas pendientes:', err)
      setError('Error al exportar las facturas pendientes')
    }
  }
  ...
  return (
    <AdminLayout title="Facturas FEL Pendientes" breadcrumbs={breadcrumbs}>
      <div className="space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
              <div className="flex flex-col sm:flex-row gap-4 flex-1">
                ...
                <Select
                  value={filters.status?.join(',') || 'draft,generated,sent'}
                  onValueChange={(value) => {
                    const statuses = value.split(',') as FelInvoiceStatus[]
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="draft,generated,sent">Todos los pendientes</SelectItem>
                    <SelectItem value="draft">Solo borradores</SelectItem>
                    <SelectItem value="generated">Solo generadas</SelectItem>
                    <SelectItem value="sent">Solo enviadas</SelectItem>
                    <SelectItem value="error">Solo con errores</SelectItem>
                  </SelectContent>
                </Select>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Facturas Pendientes de Procesamiento</CardTitle>
          </CardHeader>
          <CardContent>
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            {invoice.status === 'draft' && (
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => processPendingInvoice(invoice.id.toString())}
        ...
        {selectedInvoice && (
          <Dialog open={!!selectedInvoice} onOpenChange={() => setSelectedInvoice(null)}>
            <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Detalle de Factura Pendiente</DialogTitle>
              </DialogHeader>
              <div className="space-y-6">
                {/* Información general */}
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="mt-1">{formatDate(selectedInvoice.updatedAt)}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <div className="flex justify-end gap-2 pt-4 border-t">
                  ...
                  {selectedInvoice.status === 'draft' && (
                    <Button
                      onClick={() => processPendingInvoice(selectedInvoice.id.toString())}
                      disabled={processingInvoice === selectedInvoice.id.toString()}
                    >
                      {processingInvoice === selectedInvoice.id.toString() ? 'Procesando...' : 'Procesar Factura'}
                    </Button>
                  )}
                  {(selectedInvoice.status === 'error' || selectedInvoice.status === 'sent') && (
                    <Button
                      onClick={() => retryCertification(selectedInvoice.id.toString())}
                      disabled={processingInvoice === selectedInvoice.id.toString()}
                    >
                      {processingInvoice === selectedInvoice.id.toString() ? 'Reintentando...' : 'Reintentar'}
                    </Button>
                  )}
                </div>
              </div>
...
export default AdminFelPendingPage